...
plot <- ggplot(data = sdr_data, 
  ...
  theme_minimal() +                
  labs(    title = "SDG Scores: Goal 3 vs. Goal 4 by Region",
    x = "Goal 3 Score",
    y = "Goal 4 Score",
    color = "Region"                    
  )
...
leaflet(sdr_data_world_joined) %>% 
  ...
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("Purples", goal_12_score)(goal_12_score), label = mytext)
```
