...
library(plotly)
library(ggpubr)
p<- ggplot(sdr_data, aes(x = goal_2_score, 
                     y = goal_8_score,
...
```{r}
"My research hypothesis proposes that achieving SDG Goal 2 (Zero Hunger) will establish a positive relationship with SDG Goal 8 (Economic Growth). Countries with stronger economic growth (Goal 8) create better conditions for directing resources to food security operations and social programs which improves hunger management (Goal 2). Nations enduring persistent hunger face economic difficulties because malnutrition reduces their workforce productivity. There are specific cases where hunger does not decrease with economic growth because of inequality or poor governance yet stays minimal during moderate economic growth due to strong social policies."
```
      - are there any points that don't follow the trend (outliers)
```{r}
"Yes, some countries deviate from the expected trend."
```
        - which country(s) are outliers?
```{r}
"1.High Goal 8 (Economic Growth) but Low Goal 2 (Hunger):
United States (high economic score but persistent food insecurity)
Saudi Arabia (wealthy but reliant on food imports)
2.Low Goal 8 but High Goal 2:
Vietnam (strong agricultural policies reduce hunger despite lower GDP growth)
Moldova (effective food security programs despite economic challenges)"
"These outliers imply that  economic growth(goal 8) alone does not guarantee food security(goal 2)"
