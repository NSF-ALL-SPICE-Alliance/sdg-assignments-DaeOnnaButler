...
```{r}
ggplot(data = sdr_data, aes(x= goal_9_score, y = goal_2_score, color = goal_1_score)) +
  geom_point()
```
```{r}
# the graph repesent Goals 1,2,and 9. Goal2 shows that some countries are stuggling to meet their target with scores such as (0,20,40); While goals like Goal1 have a broader range varying from(0-100). same for goal 9
```
### Bar Chart
...
ggplot(sdr_data, aes(x = goal_10_score, fill=regions_used_for_the_sdr)) + 
  geom_histogram(bins=33, color="black")
```
```{r}
# This graph is probably showing how different parts of the world stack up on Goal 10. Most of the scores are packed down near 0-25, meaning a bunch of regions are not the best at fixing inequality. The higher scores (like 75-100) are probably rich places like the OECD, while Sub-Saharan Africa and others are stuck at the bottom. There’s also an "NA".
```
